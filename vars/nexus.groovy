def nexus(COMPONENT) {
  ...
  command = "curl -f -v -u admin:admin --upload-file ${FILENAME} http://172.31.3.145:8081/repository/${COMPONENT}/${FILENAME}"
...
def code_build(APP_TYPE, COMPONENT) {
  if(APP_TYPE == "NODEJS") {
    ...
    command = "export GOBIN=$GOPATH/bin && go get &>>$LOG && go build"
    def execute_com=sh(returnStdout: true, script: command)
    print execute_com
